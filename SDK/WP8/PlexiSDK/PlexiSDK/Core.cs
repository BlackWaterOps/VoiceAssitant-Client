...
namespace PlexiSDK
{
    ...
    {
        /// <summary>
        /// Gets the original query that initiated the request
        /// </summary>
        string OriginalQuery { get; }

        /// <summary>
        /// Gets to the location data about the current query if it exists
        /// </summary>
        GeoLocation QueryLocation { get; }
    ...
    public class Core : IPlexiService
    {
        ...
        public GeoLocation QueryLocation { get; private set; }
        ...
        public void Query(string query)
        {
            ...
            PlexiSDK.State currState = currentState.State;
            PlexiSDK.State newState;

            switch (currState)
            {
                case PlexiSDK.State.InProgress:
                case PlexiSDK.State.Error:
                    newState = PlexiSDK.State.DisambiguateActive; 
                    break;

                case PlexiSDK.State.Choice:
                    newState = PlexiSDK.State.DisambiguateCandidate;
                    break;

                default:
                    newState = PlexiSDK.State.Init;
                    break;
            }
        ...
        {
            this.QueryLocation = (this.mainContext.payload.ContainsKey("location")) ? this.mainContext.payload["location"].DeepCopy<GeoLocation>() : null;
