...
{
    public class ImagesViewModel : GalaSoft.MvvmLight.ViewModelBase, IViewModel
    {
        ...
        public ImagesViewModel(INavigationService navigationService, IPleaseService pleaseService)
        {
        ...
        public void ImageTest()
        {
            try
            {
                ...
                ImageList = JsonConvert.DeserializeObject<List<object>>(test);
        ...
        public Dictionary<string, object> Populate(string templateName, Dictionary<string, object> structured)
        {
            ...
            {
                imageList = ((JArray)structured["items"]).ToObject<List<object>>();

                // return title, subtitle, and page back to service so we can be decoupled from singleviewmodel

                ret.Add("title", "image results");
                ret.Add("subtitle", String.Format("image search on \"{0}\"", this.pleaseService.OriginalQuery));
                ret.Add("page", ViewModelLocator.ImagesPageUri);
            }
