...
namespace Microsoft.Phone.Controls
{
    ...
    public class ThumbnailedImageViewer : Control
    {
        ...
        {
            if (DataContext is IThumbnailedImage == false && DataContext is IThumbnailedImageAsync == false)
            {
                return;
            }
            ...
            if (DataContext is IThumbnailedImageAsync)
            {
                var source = await ((IThumbnailedImageAsync)DataContext).GetThumbnailImage();
                _thumbnailBitmapImage.SetSource(source);
            }
            else
            {
        ...
        private async void BeginLoadingFullResolutionImage()
        {
            ...
            if (DataContext is IThumbnailedImageAsync)
            {
                var source = await ((IThumbnailedImageAsync)DataContext).GetImage();
                _fullResolutionBitmapImage.SetSource(source);
            }
            else
            {
